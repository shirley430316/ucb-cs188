...
def constructBayesNet(gameState: hunters.GameState):
    ...
    possible_obs = range(X_RANGE + Y_RANGE + 6)
    variableDomainsDict[OBS0] = possible_obs
    variableDomainsDict[OBS1] = possible_obs
    "*** END YOUR CODE HERE ***"

    net = bn.constructEmptyBayesNet(variables, edges, variableDomainsDict)
    return net

def getPossiblePosition(pacmanPos, dist, X_RANGE, Y_RANGE):
    possible_pos = []
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            if manhattanDistance((x, y), pacmanPos) < abs(dist+7):
                possible_pos.append((x, y))
    return possible_pos
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        # If all weights are zero, return None
        if total == 0:
            return None
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            for oldPos in self.allPositions:
                newDist[pos] += self.getPositionDistribution(gameState, oldPos)[pos] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        baseCount = self.numParticles // numPositions
        ...
        for i in range(numPositions):
            ...
            if i < remainder:
                count += 1
            self.particles.extend([legalPositions[i]] * count)
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            # Get transition distribution for this particle
            newPosDist = self.getPositionDistribution(gameState, oldParticle)
        
            # Sample one new position from the distribution
            newParticle = newPosDist.sample()
        
            # Add to new particle list
            newParticles.append(newParticle)
    
        # Update particles with new time step
        self.particles = newParticles
